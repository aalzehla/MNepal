...
{
    public class ServicesLimitCheck
    {
        public string ServiceLimitCheck(string mobile, string destmobile, string amount, string sc, string pin, string src)
        {
            int transCount = 10;
            ...
            if (customerNo != "0")
            {
                ...
                if (pin != "")
                {
                    ...
                    if (validStatus == "Active")
                    {
                        ...
                        /// 31 - W2B Utility Payment
                        /// 32 - W2B Coupon Recharge
                        if ((sc == "30") || (sc == "31") || (sc == "32"))
                        {
                            //Transaction Limit Wallet
                            DataTable dtTransWalletLimit = WalletLimitUtils.GetSuperWalletTxnLimit(mobile, "Services");
