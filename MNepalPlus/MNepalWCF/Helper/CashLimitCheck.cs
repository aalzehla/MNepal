...
{
    public class CashLimitCheck
    {
        public string CILimitCheck(string mobile, string destmobile, string amount, string sc, string pin, string src)
        {
            int transCount = 10;
            ...
            if (customerNo != "0")
            {
                ...
                if (pin != "")
                {
                    ...
                    {
                        /// 50 - Cash In Wallet to Wallet
                        if (sc == "50")
                        {
                            //Transaction Limit Wallet
                            DataTable dtTransWalletLimit = WalletLimitUtils.GetSuperWalletTxnLimit(mobile, "Cash In");
        ...
        public string COLimitCheck(string mobile, string destmobile, string amount, string sc, string pin, string src)
        {
            int transCount = 10;
            decimal singleTransLimit = 500;
            decimal DailyLimit = 5000;
            ...
            if (customerNo != "0")
            {
                ...
                if (pin != "")
                {
                    ...
                    if (validStatus == "Active")
                    {
                        /// 51 - Cash Out Wallet to Wallet
                        if (sc == "51")
                        {
                            //Transaction Limit Wallet
                            DataTable dtTransWalletLimit = WalletLimitUtils.GetSuperWalletTxnLimit(mobile, "Cash Out");
                            if (dtTransWalletLimit.Rows.Count > 0)
                            {
                                DailyLimit = Convert.ToDecimal(dtTransWalletLimit.Rows[0]["WPerDayAmt"].ToString() == "" ? "0" : dtTransWalletLimit.Rows[0]["WPerDayAmt"].ToString());
                                MonthlyLimit = Convert.ToDecimal(dtTransWalletLimit.Rows[0]["WTxnAmtM"].ToString() == "" ? "0" : dtTransWalletLimit.Rows[0]["WTxnAmtM"].ToString());
