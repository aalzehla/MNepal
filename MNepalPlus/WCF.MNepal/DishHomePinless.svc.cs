...
namespace WCF.MNepal
{
    ...
    public class DishHomePinless
    {
        [OperationContract]
        #region"Check Paypoint Dish Home"
        ...
        public string checkpayment(Stream input)
        {
            ...
            string note = "utility payment for DishHome Pinless. CAS ID/Chip ID/Customer ID=" + qs["account"];
            string src = qs["src"];
            string result = "";
            ...
            PaypointModel reqCPPaypointDishHomeInfo = new PaypointModel();//to store data request of CP which is also commmon in nea
            PaypointModel resCPPaypointDishHomeInfo = new PaypointModel();//to store data response of CP which is also commmon in nea

            //PaypointModel resPaypointPaymentInfo = new PaypointModel();
            PaypointModel resPaypointDishHomePaymentInfo = new PaypointModel();//to store data of Response of CP only
            ...
            try
            {
                ...
                reqCPPaypointDishHomeInfo = new PaypointModel()
                {
                ...
                using (WebClient wc = new WebClient())
                {
                    ...
                    if (xElem.Attribute("Result").Value == "000")
                    {
                        ...
                        if (mask == "0" || mask == "6")
                        {
                            ...
                            string stringBuilderBonus = "";
                            foreach (XmlNode xmlNode in xmlNodeList)
                            {
                                packages.Description = xmlNode.OuterXml; /*xmlNode.InnerText;*/
                                packages.Bonus = xmlNode.Attributes["bonus"].Value;
                                ...
                                stringBuilderBonus = stringBuilderBonus + packages.Bonus + Environment.NewLine;

                            }
                            resPaypointDishHomePaymentInfo = new PaypointModel()
                            {
                                description = stringBuilderDescriptions,
                                amountP = stringBuilderAmounts,
                                Bonus = stringBuilderBonus,
                        ...
                        int resultsPayments = PaypointUtils.PaypointDishHomeInfo(resPaypointDishHomePaymentInfo);
                    }
                    else
                    {
                        keyrlt = xElem.Attribute("Key").Value;
                    ...
                    resCPPaypointDishHomeInfo = new PaypointModel()
                    {
            ...
            try
            {
                int resultsReqCP = PaypointUtils.RequestCPPaypointInfo(reqCPPaypointDishHomeInfo);
                int resultsResCP = PaypointUtils.ResponseCPPaypointInfo(resCPPaypointDishHomeInfo);
            ...
            else if (statusCode == "200")
            {
                ...
                var v = new
                {
                    ...
                    retrievalRef = resCPPaypointDishHomeInfo.retrievalReferenceResCP,
                    refStanCK = resCPPaypointDishHomeInfo.refStanResCP,
                    description = pkg
                };
        ...
        [WebInvoke(Method = "POST",
                 ResponseFormat = WebMessageFormat.Json)]
        public string DishHomePinCheckpayment(Stream input)
        {
            string PaypointPwd = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointPwd"];
            string PaypointUserID = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointUserID"];
            System.Net.ServicePointManager.ServerCertificateValidationCallback += delegate { return true; }; //to prevent from SSL error
            ...
            try
            {
                ...
                using (WebClient wc = new WebClient())
                {
                    ...
                    if (xElem.Attribute("Result").Value == "000")
                    {
                        ...
                        int resultsPayments = PaypointUtils.PaypointDishHomePinInfo(resPaypointDishHomePaymentInfo);
                    }
                    else
                    {
                        keyrlt = xElem.Attribute("Key").Value;
        ...
        [WebInvoke(Method = "POST",
                  ResponseFormat = WebMessageFormat.Json)]
        public string DHPinExecutepayment(Stream input)
        {
            string PaypointPwd = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointPwd"];
            string PaypointUserID = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointUserID"];
            ...
            string amount = qs["amount"];//amount paid by customer 
            string pin = qs["pin"];
            string note = "utility payment for Dish Home Pin. Customer Name=" + qs["account"];//+ ". " + qs["note"];
            string src = qs["src"];
            string result = "";
            string sessionID = qs["tokenID"];
            string resultMessageResCP = "";
            ...
            PaypointModel reqEPPaypointDishHomeInfo = new PaypointModel();
            PaypointModel resEPPaypointDishHomeInfo = new PaypointModel();
            PaypointModel resEPPaypointDishHomePinInfo = new PaypointModel();
                ...
                if (statusCode == "200")
                {
                    try
                    {
                        ...
                        if (rltCheckPaymt == "000")//go if CP Response is 000
                        {
                            ...
                            do
                            {
                                ...
                                else
                                {
                                    ...
                                    using (WebClient wcExecPay = new WebClient())
                                    {
                                        ...
                                        var ResultData = xElemEPay.Descendants("ResultData").SingleOrDefault();  //holds result for voucher code and id
                                        XmlDocument xmlDoc1 = new XmlDocument();
                                        xmlDoc1.LoadXml(ResultData.ToString());
                                        XmlNodeList xmlNodeList = xmlDoc1.SelectNodes("/ResultData/Voucher");
                                        
                                        string VoucherDetails = "";
                                        foreach (XmlNode xmlNode in xmlNodeList)
                                        {
                                            VoucherDetails = xmlNode.OuterXml; 
                                        }
                                        //
                                        var charsToRemove = new string[] { "<", ">", "/", "/>", "\"" };
                                        foreach (var c in charsToRemove)
                                        {
                                            VoucherDetails = VoucherDetails.Replace(c, string.Empty);
                                        }


                                        string[] substrings = Regex.Split(VoucherDetails, @"id");  //string split


                                        VoucherCode = substrings[0].ToString();  //VoucherCode
                                        VoucherCode = VoucherCode.Replace("Voucher code=", "");
                                        Id =substrings[1].ToString();  //VoucherId
                                        Id = Id.Replace("=", "");
                                        ...
                                        //for response Execute payment Pin info
                                        
                                        resEPPaypointDishHomePinInfo = new PaypointModel()
                                        {
                            ...
                            ///for  Inserting EP PayPoint Data for DH
                            ...
                            try
                            {
                                ...
                                int resultsResPinInfo = PaypointUtils.ResponseEPPaypointDHPinInfo(resEPPaypointDishHomePinInfo);  //Dish Home Pin Info
                                        ...
                                        if (response1.StatusCode == HttpStatusCode.OK)
                                        {
                                            ...
                                            try
                                            {
                                                ...
                                                messagereply += " You have successfully reverse  NPR " + validTransactionData.Amount
                                                                    + " to " +
                                                                    //GetMerchantName 
                                                                    "Utility payment for Dish Home Pin." + " on date " +
                                                                    (validTransactionData.CreatedDate).ToString("dd/MM/yyyy")
                                                                + "." + "\n";
                                                messagereply += "Thank you. MNepal";
                ...
                if (compResultResp == "000")
                {
                    ...
                    if (response2.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        try
                        {
                            ...
                            messagereply += "Payment Success to Utility payment for DishHome Pin. " +
                                            //GetMerchantName 
                                             " on date " +
                                                (validTransactionData.CreatedDate).ToString("dd/MM/yyyy")
                                            + "." + " Voucher Code: " + VoucherCode + "Id: " + Id + "\n";
                            messagereply += "Thank you. MNepal";
