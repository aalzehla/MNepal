...
namespace WCF.MNepal
{
    ...
    public class ISP
    {
        #region"Check Paypoint WebSurfer"
        [OperationContract]
        [WebInvoke(Method = "POST",
                  ResponseFormat = WebMessageFormat.Json)]
        public string WebSurferCheckPayment(Stream input)
        {
            ...
            PaypointModel reqCPPaypointWebSurferInfo = new PaypointModel();//to store data request of CP which is also commmon in nea
            PaypointModel resCPPaypointWebSurferInfo = new PaypointModel();//to store data response of CP which is also commmon in nea

            //PaypointModel resPaypointPaymentInfo = new PaypointModel();
            PaypointModel resPaypointWebSurferPaymentInfo = new PaypointModel();//to store data of Response of CP only
            ...
            try
            {
                ...
                reqCPPaypointWebSurferInfo = new PaypointModel()
                {
                ...
                using (WebClient wc = new WebClient())
                {
                    ...
                    if (xElem.Attribute("Result").Value == "000")
                    {
                        ...
                        if (mask == "0" || mask == "6" || mask == "3")
                        {
                            ...
                            resPaypointWebSurferPaymentInfo = new PaypointModel()
                            {
                        ...
                        int resultsPayments = PaypointUtils.PaypointWebSurferPaymentInfo(resPaypointWebSurferPaymentInfo);
                    }
                    else
                    {
                        keyrlt = xElem.Attribute("Key").Value;
                    ...
                    resCPPaypointWebSurferInfo = new PaypointModel()
                    {
            ...
            try
            {
                int resultsReqCP = PaypointUtils.RequestCPPaypointInfo(reqCPPaypointWebSurferInfo);
                int resultsResCP = PaypointUtils.ResponseCPPaypointInfo(resCPPaypointWebSurferInfo);
            ...
            else if (statusCode == "200")
            {
                ...
                var v = new
                {
                    ...
                    retrievalRef = resCPPaypointWebSurferInfo.retrievalReferenceResCP,
                    refStanCK = resCPPaypointWebSurferInfo.refStanResCP,
                    description = pkg,
                    user = users
                };
        ...
        #region"Check Paypoint ArrowNet"
        [OperationContract]
        [WebInvoke(Method = "POST",
                  ResponseFormat = WebMessageFormat.Json)]
        public string ArrowNetCheckPayment(Stream input)
        {
            string PaypointPwd = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointPwd"];
            string PaypointUserID = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointUserID"];
            System.Net.ServicePointManager.ServerCertificateValidationCallback += delegate { return true; }; //to prevent from SSL error
            ...
            PaypointModel reqCPPaypointArrowNetInfo = new PaypointModel();//to store data request of CP which is also commmon in nea
            PaypointModel resCPPaypointArrowNetInfo = new PaypointModel();//to store data response of CP which is also commmon in nea

            //PaypointModel resPaypointPaymentInfo = new PaypointModel();
            PaypointModel resPaypointArrowNetPaymentInfo = new PaypointModel();//to store data of Response of CP only
            ...
            try
            {
                ...
                //for checkpayment request insert in database
                reqCPPaypointArrowNetInfo = new PaypointModel()
                {
                    companyCodeReqCP = companyCode,
                    serviceCodeReqCP = serviceCode,
                    accountReqCP = account,
                ...
                using (WebClient wc = new WebClient())
                {
                    ...
                    if (xElem.Attribute("Result").Value == "000")
                    {
                        ...
                        if (mask == "0" || mask == "6" || mask == "3")
                        {
                            ...
                            resPaypointArrowNetPaymentInfo = new PaypointModel()
                            {
                                description = stringBuilderDescriptions,
                                amountP = stringBuilderAmounts,
                                PackageId = stringBuilderPackageId,
                        ...
                        int resultsPayments = PaypointUtils.PaypointArrowNetPaymentInfo(resPaypointArrowNetPaymentInfo);
                    }
                    else
                    {
                        keyrlt = xElem.Attribute("Key").Value;
                    ...
                    //for checkpayment response insert in database in nepal water
                    resCPPaypointArrowNetInfo = new PaypointModel()
                    {
                        companyCodeResCP = companyCode,
                        serviceCodeResCP = serviceCode,
                        accountResCP = account,
            ...
            try
            {
                int resultsReqCP = PaypointUtils.RequestCPPaypointInfo(reqCPPaypointArrowNetInfo);
                int resultsResCP = PaypointUtils.ResponseCPPaypointInfo(resCPPaypointArrowNetInfo);
            ...
            else if (statusCode == "200")
            {
                ...
                var v = new
                {
                    StatusCode = Convert.ToInt32(statusCode),
                    StatusMessage = failedmessage,
                    retrievalRef = resCPPaypointArrowNetInfo.retrievalReferenceResCP,
                    refStanCK = resCPPaypointArrowNetInfo.refStanResCP,
                    description = pkg,
                };
                result = JsonConvert.SerializeObject(v);
            }
