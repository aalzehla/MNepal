...
namespace WCF.MNepal
{
    ...
    public class TVServicePayment
    {
        #region"Check Paypoint SIMTV"
        [OperationContract]
        [WebInvoke(Method = "POST",
                  ResponseFormat = WebMessageFormat.Json)]
        public string SIMTVcheckpayment(Stream input)
        {
            ...
            PaypointModel reqCPPaypointSIMTVInfo = new PaypointModel();//to store data request of CP which is also commmon in nea
            PaypointModel resCPPaypointSIMTVInfo = new PaypointModel();//to store data response of CP which is also commmon in nea

            //PaypointModel resPaypointPaymentInfo = new PaypointModel();
            PaypointModel resPaypointSIMTVPaymentInfo = new PaypointModel();//to store data of Response of CP only
            ...
            try
            {
                ...
                reqCPPaypointSIMTVInfo = new PaypointModel()
                {
                ...
                using (WebClient wc = new WebClient())
                {
                    ...
                    if (xElem.Attribute("Result").Value == "000")
                    {
                        ...
                        if (mask == "0" || mask == "6")
                        {
                            ...
                            resPaypointSIMTVPaymentInfo = new PaypointModel()
                            {
                        ...
                        int resultsPayments = PaypointUtils.PaypointSIMTVPaymentInfo(resPaypointSIMTVPaymentInfo);
                    }
                    else
                    {
                        keyrlt = xElem.Attribute("Key").Value;
                    ...
                    resCPPaypointSIMTVInfo = new PaypointModel()
                    {
            ...
            try
            {
                int resultsReqCP = PaypointUtils.RequestCPPaypointInfo(reqCPPaypointSIMTVInfo);
                int resultsResCP = PaypointUtils.ResponseCPPaypointInfo(resCPPaypointSIMTVInfo);
            ...
            else if (statusCode == "200")
            {
                ...
                var v = new
                {
                    ...
                    retrievalRef = resCPPaypointSIMTVInfo.retrievalReferenceResCP,
                    refStanCK = resCPPaypointSIMTVInfo.refStanResCP,
                    description = pkg
                };
        ...
        [WebInvoke(Method = "POST",
                  ResponseFormat = WebMessageFormat.Json)]
        public string SIMTVexecutepayment(Stream input)
        {
            string PaypointPwd = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointPwd"];
            string PaypointUserID = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointUserID"];
                    ...
                    if (response2.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        try
                        {
                            ...
                            messagereply += " You have successfully paid NPR " + validTransactionData.Amount
                                            ...
                                            //GetMerchantName 
                                            "Utility payment for SIMTV." + " on date " +
                                                (validTransactionData.CreatedDate).ToString("dd/MM/yyyy")
                                            + "." + "\n";
                            messagereply += "Thank you. MNepal";
        ...
        #region"Check Paypoint MeroTV"
        [OperationContract]
        [WebInvoke(Method = "POST",
                  ResponseFormat = WebMessageFormat.Json)]
        public string MeroTVcheckpayment(Stream input)
        {
            string PaypointPwd = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointPwd"];
            string PaypointUserID = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointUserID"];
            System.Net.ServicePointManager.ServerCertificateValidationCallback += delegate { return true; }; //to prevent from SSL error
            ...
            string mobile = qs["mobile"];
            string da = System.Web.Configuration.WebConfigurationManager.AppSettings["DestinationNoForTestServer"];
            string note = "utility payment for MeroTV. Customer ID=" + qs["account"];//+ ". " + qs["note"];
            ...
            PaypointModel reqCPPaypointMeroTVInfo = new PaypointModel();//to store data request of CP which is also commmon in nea
            PaypointModel resCPPaypointMeroTVInfo = new PaypointModel();//to store data response of CP which is also commmon in nea

            //PaypointModel resPaypointPaymentInfo = new PaypointModel();
            PaypointModel resPaypointMeroTVPaymentInfo = new PaypointModel();//to store data of Response of CP only
            ...
            try
            {
                ...
                //for checkpayment request insert in database
                reqCPPaypointMeroTVInfo = new PaypointModel()
                {
                    companyCodeReqCP = companyCode,
                    serviceCodeReqCP = serviceCode,
                    accountReqCP = account,
                ...
                using (WebClient wc = new WebClient())
                {
                    ...
                    if (xElem.Attribute("Result").Value == "000")
                    {
                        ...
                        if (mask == "0" || mask == "6" || mask=="3")
                        {
                            ...
                            var package = xElem.Descendants("packages").SingleOrDefault();
                            var smartcards = xElem.Descendants("smartcards").SingleOrDefault();
                            //var packageList = package.Descendants("package").ToList();
                            ...
                            xmlDoc2.LoadXml(smartcards.ToString());

                            XmlNodeList xmlNodeList = xmlDoc1.SelectNodes("/packages/package");
                            XmlNodeList smartcardsxmlNodeList = xmlDoc2.SelectNodes("/smartcards/sc");

                            string stringBuilderDescriptions = "";
                            string stringBuilderAmounts = "";
                            string stringBuilderPackageId = "";
                            string stringBuilderSmartCards = "";
                            foreach (XmlNode xmlNode in xmlNodeList)
                            {
                                packages.Description = xmlNode.OuterXml; /*xmlNode.InnerText;*/
                            ...
                            foreach (XmlNode xmlNode in smartcardsxmlNodeList)
                            {
                                cards.smartCards = xmlNode.OuterXml;
                                smartCards.Add(cards);
                                stringBuilderSmartCards = stringBuilderSmartCards + cards.smartCards + Environment.NewLine;
                                
                            }
                            resPaypointMeroTVPaymentInfo = new PaypointModel()
                            {
                                description = stringBuilderDescriptions,
                                amountP = stringBuilderAmounts,
                                PackageId = stringBuilderPackageId,
                                smartCards = stringBuilderSmartCards,
                                billNumber = billNumber,
                                refStan = refStan,
                        ...
                        int resultsPayments = PaypointUtils.PaypointMeroTVPaymentInfo(resPaypointMeroTVPaymentInfo);
                    }
                    else
                    {
                        keyrlt = xElem.Attribute("Key").Value;
                    ...
                    //for checkpayment response insert in database in nepal water
                    resCPPaypointMeroTVInfo = new PaypointModel()
                    {
                        companyCodeResCP = companyCode,
                        serviceCodeResCP = serviceCode,
                        accountResCP = account,
            ...
            try
            {
                int resultsReqCP = PaypointUtils.RequestCPPaypointInfo(reqCPPaypointMeroTVInfo);
                int resultsResCP = PaypointUtils.ResponseCPPaypointInfo(resCPPaypointMeroTVInfo);
            ...
            else if (statusCode == "200")
            {
                ...
                var v = new
                {
                    StatusCode = Convert.ToInt32(statusCode),
                    StatusMessage = failedmessage,
                    retrievalRef = resCPPaypointMeroTVInfo.retrievalReferenceResCP,
                    refStanCK = resCPPaypointMeroTVInfo.refStanResCP,
                    description = pkg,
        ...
        [WebInvoke(Method = "POST",
                  ResponseFormat = WebMessageFormat.Json)]
        public string MeroTVexecutepayment(Stream input)
        {
            string PaypointPwd = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointPwd"];
            string PaypointUserID = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointUserID"];
            ...
            string amount = qs["amount"];//amount paid by customer 
            string pin = qs["pin"];
            string note = "utility payment for MeroTV. Customer Name=" + qs["account"];//+ ". " + qs["note"];
            string src = qs["src"];
            string result = "";
            string sessionID = qs["tokenID"];
            string resultMessageResCP = "";
                    ...
                    if (response2.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        try
                        {
                            ...
                            messagereply += " You have successfully paid NPR " + validTransactionData.Amount
                                            + " to " + " account name: " + account + ". " +
                                            //GetMerchantName 
                                            "Utility payment for MeroTV." + " on date " +
                                                (validTransactionData.CreatedDate).ToString("dd/MM/yyyy")
                                            + "." + "\n";
                            messagereply += "Thank you. MNepal";
        ...
        #region"Check Paypoint SkyTV"
        [OperationContract]
        [WebInvoke(Method = "POST",
                  ResponseFormat = WebMessageFormat.Json)]
        public string Skycheckpayment(Stream input)
        {
            string PaypointPwd = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointPwd"];
            string PaypointUserID = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointUserID"];
            System.Net.ServicePointManager.ServerCertificateValidationCallback += delegate { return true; }; //to prevent from SSL error
            ...
            string mobile = qs["mobile"];
            string da = System.Web.Configuration.WebConfigurationManager.AppSettings["DestinationNoForTestServer"];
            string note = "utility payment for SkyTV. Customer ID=" + qs["account"];//+ ". " + qs["note"];
            ...
            PaypointModel reqCPPaypointSkyTVInfo = new PaypointModel();//to store data request of CP which is also commmon in nea
            PaypointModel resCPPaypointSkyTVInfo = new PaypointModel();//to store data response of CP which is also commmon in nea

            //PaypointModel resPaypointPaymentInfo = new PaypointModel();
            PaypointModel resPaypointSkyTVPaymentInfo = new PaypointModel();//to store data of Response of CP only
            ...
            try
            {
                ...
                //for checkpayment request insert in database
                reqCPPaypointSkyTVInfo = new PaypointModel()
                {
                    companyCodeReqCP = companyCode,
                    serviceCodeReqCP = serviceCode,
                    accountReqCP = account,
                ...
                using (WebClient wc = new WebClient())
                {
                    ...
                    if (xElem.Attribute("Result").Value == "000")
                    {
                        ...
                        if (mask == "0" || mask == "6" || mask == "3")
                        {
                            ...
                            if (smartcards != null)
                            {
                                ...
                                if (special1 == "1")
                                {
                                    XmlNodeList smartcardsxmlNodeList = xmlDoc2.SelectNodes("/smartcards/smartcard");
                                    foreach (XmlNode xmlNode in smartcardsxmlNodeList)
                                    {
                                        cards.smartCards = xmlNode.OuterXml;
                                        smartCards.Add(cards);
                            ...
                            resPaypointSkyTVPaymentInfo = new PaypointModel()
                            {
                                description = stringBuilderDescriptions,
                                amountP = stringBuilderAmounts,
                                PackageId = stringBuilderPackageId,
                                smartCards = stringBuilderSmartCards,
                                ftthUser = stringBuilderUsers,
                                billNumber = billNumber,
                                reserveInfo = reserveInfo,
                                refStan = refStan,
                                amount = amountpay,
                        ...
                        int resultsPayments = PaypointUtils.PaypointSkyTVPaymentInfo(resPaypointSkyTVPaymentInfo);
                    }
                    else
                    {
                        keyrlt = xElem.Attribute("Key").Value;
                    ...
                    //for checkpayment response insert in database in nepal water
                    resCPPaypointSkyTVInfo = new PaypointModel()
                    {
                        companyCodeResCP = companyCode,
                        serviceCodeResCP = serviceCode,
                        accountResCP = account,
            ...
            try
            {
                int resultsReqCP = PaypointUtils.RequestCPPaypointInfo(reqCPPaypointSkyTVInfo);
                int resultsResCP = PaypointUtils.ResponseCPPaypointInfo(resCPPaypointSkyTVInfo);
            ...
            else if (statusCode == "200")
            {
                ...
                var v = new
                {
                    StatusCode = Convert.ToInt32(statusCode),
                    StatusMessage = failedmessage,
                    retrievalRef = resCPPaypointSkyTVInfo.retrievalReferenceResCP,
                    refStanCK = resCPPaypointSkyTVInfo.refStanResCP,
                    description = pkg,
                    smartCards = smartCards
                };
                result = JsonConvert.SerializeObject(v);
        ...
        #region"Check Paypoint NetTv"
        [OperationContract]
        [WebInvoke(Method = "POST",
                  ResponseFormat = WebMessageFormat.Json)]
        public string NetTvCheckpayment(Stream input)
        {
            string PaypointPwd = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointPwd"];
            string PaypointUserID = System.Web.Configuration.WebConfigurationManager.AppSettings["PaypointUserID"];
            System.Net.ServicePointManager.ServerCertificateValidationCallback += delegate { return true; }; //to prevent from SSL error
            ...
            string mobile = qs["mobile"];
            string da = System.Web.Configuration.WebConfigurationManager.AppSettings["DestinationNoForTestServer"];
            string note = "utility payment for NetTv. Customer ID=" + qs["account"];//+ ". " + qs["note"];
            ...
            try
            {
                ...
                string myParameters = "companyCode=" + companyCode + "&serviceCode=" + serviceCode +
                    "&account=" + account + "&special1=" + amount + "&special2=" + special2 +
                    "&transactionDate=" + transactionDate + "&transactionId=" + transactionId +
                    "&userId=" + userId.Trim() + "&userPassword=" + userPassword.Trim() + "&salePointType=" + salePointType;
