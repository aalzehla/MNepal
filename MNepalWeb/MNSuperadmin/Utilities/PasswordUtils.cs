...
namespace MNSuperadmin.Utilities
{
    public static class PasswordUtils
    {
        ...
        #endregion
        #region "Member Change Password"

        public static bool ResetPassword(this UserInfo member)
        {
            var objMemberInfo = new UserInfo()
            {
                Password = member.Password,
                ClientCode = member.ClientCode,
                Mode = "NPWD" // Change Password
            };
            return new PasswordUserModel().ResetPassword(objMemberInfo) > 0;
        }
