...
namespace MNSuperadmin.Controllers
{
    ...
    public class SuperAdminRoleSetupController : Controller
    {
        ///<summary>
            ...
            if (TempData["userType"] != null)
            {
                ...
                TempData["admin_messsage"] = result
                    ...
                    : "Error while inserting the information. ERROR :: "
                      + errorMessage;
                TempData["message_class"] = result ? "success_info" : "failed_info";
                return RedirectToAction("Index", "SuperAdminRoleSetup");
            }
            else
            {
                return RedirectToAction("Index", "Login");
        ...
        public ActionResult CreateAdminProfile(AdminProfileVM model)
        {
            ...
            if (TempData["userType"] != null)
            {
                ...
                if (result)
                    return RedirectToAction("CreateAdminProfile", "SuperAdminRoleSetup");
                else
                    return View(model);
            }
            else
        ...
        public ActionResult EditAdminProfileSetup(string profileCode)
        {
            ...
            if (TempData["userType"] != null)
            {
                ...
                if (dtblUserInfo.Rows.Count == 1)
                {
                    adminProfile.ProfileCode = dtblUserInfo.Rows[0]["SProfileCode"].ToString();
                    adminProfile.ProfileName = dtblUserInfo.Rows[0]["SProfileName"].ToString();
                    adminProfile.ProfileDesc = dtblUserInfo.Rows[0]["SProfileDesc"].ToString();
                    adminProfile.ProfileStatus = dtblUserInfo.Rows[0]["SProfileStatus"].ToString();
                }
                else
                {
        ...
        public bool CheckSuperAdminProfile(string ProfileName)
        {
            if (!String.IsNullOrEmpty(ProfileName))
            {

                DataTable ProfileInfo = UserProfileUtils.GetSuperAdminUserProfile(ProfileName);
