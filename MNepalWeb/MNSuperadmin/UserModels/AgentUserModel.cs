...
namespace MNSuperadmin.UserModels
{
    public class AgentUserModel
    {
        ...
        internal List<UserInfo> GetAgentModApprove(UserInfo objUserInfo, string UserName)
        {
            ...
            try
            {
                using (conn = new SqlConnection(DatabaseConnection.ConnectionStr()))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        string Command = @"SELECT * FROM dbo.v_MNClientDetail WHERE UserType = 'agent' AND IsApproved = 'UnApprove' AND IsModified = 'T' AND Status = 'InActive' AND ISNULL(IsRejected,'F')='F'";
        ...
        public int AgentModApprove(UserInfo objUserInfo, string Approve)
        {
            SqlConnection conn = null;
            int ret;
            try
            {
                using (conn = new SqlConnection(DatabaseConnection.ConnectionStr()))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        cmd.CommandText = @"UPDATE MNClient SET Status = 'Active', IsApproved = 'Approve', IsModified = 'F',ApprovedDate = @ApprovedDate, ApprovedBy = @ApprovedBy where ClientCode = @ClientCode";
