...
jQuery(document).ready(function ($) {
    ...
    $('#BSCitizenshipIssueDate, #BSLicenseIssueDate').nepaliDatePicker({
        ...
        onChange: function () {
            ...
            if ($('#BSLicenseIssueDate').val() != '') {
                $('#LicenseIssueDate').val(GetShowFormattedDate(BS2AD(GetFormattedDate($('#BSLicenseIssueDate').val()))));
            }
    ...
    $('#BSLicenseExpireDate').nepaliDatePicker({
        ...
        onChange: function () {
            $('#LicenseExpireDate').val(GetShowFormattedDate(BS2AD(GetFormattedDate($('#BSLicenseExpireDate').val()))));
                        // convert to BS to AD and set value
        }
    });
