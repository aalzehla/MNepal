...
namespace MNepalWeb.UserModels
{
    public class CustApproveUserModels
    {
        ...
        internal List<UserInfo> GetAdminApproveDetailInfo(UserInfo objUserInfo, string UserName)
        {
            ...
            try
            {
                using (conn = new SqlConnection(DatabaseConnection.ConnectionStr()))
                {
                    ...
                    {
                        string Command = @"SELECT * FROM dbo.v_MNClientDetail WHERE UserType = 'admin' AND IsApproved = 'UnApprove' AND Status = 'InActive' AND ISNULL(IsRejected,'F')='F'";


                        if (!string.IsNullOrEmpty(UserName))
                        {
                            Command = Command + " AND LOWER(UserName) like LOWER(LTRIM(RTRIM('" + UserName + "')))+'%'";
                        }
        ...
        #region BankLink List
        internal DataTable GetBankLinkApproveDetailInfo(UserInfo objUserInfo)//CustApproveInfo
        {
            Database database;
            DataTable dtableResult = null;
            /*
            ...
            */
            ...
            {
                database = DatabaseConnection.GetDatabase();
                using (var command = database.GetStoredProcCommand("[s_MNBankLinkInfoApprove]"))
                {
                    //database.AddInParameter(command, "@UserType", DbType.String, objUserInfo.UserType);
                    database.AddInParameter(command, "@ClientCode", DbType.String, objUserInfo.ClientCode);
                    database.AddInParameter(command, "@mode", DbType.String, objUserInfo.Mode);
        ...
        internal int BankLinkApprove(UserInfo objUserInfo)
        {
            SqlConnection sqlCon = null;
            int ret;
            try
            {
                ...
                {
                    sqlCon.Open();
                    using (SqlCommand sqlCmd = new SqlCommand("[s_MNLinkBankAcc]", sqlCon))
                    {
                        sqlCmd.CommandType = CommandType.StoredProcedure;
        ...
        internal int BankLinkReject(UserInfo objUserInfo)
        {
            SqlConnection sqlCon = null;
            int ret;
            try
            {
                ...
                {
                    sqlCon.Open();
                    using (SqlCommand sqlCmd = new SqlCommand("[s_MNRejectLinkBankAcc]", sqlCon))
                    {
                        sqlCmd.CommandType = CommandType.StoredProcedure;
        ...
        ////retrieve AccNo
        internal DataTable GetAccNo(UserInfo objUserInfo)
        {
            Database database;
            DataTable dtableResult = null;
